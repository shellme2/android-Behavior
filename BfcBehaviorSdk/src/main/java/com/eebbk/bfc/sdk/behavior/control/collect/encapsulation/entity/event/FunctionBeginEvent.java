...
public class FunctionBeginEvent extends AEvent<FunctionBeginEvent>{
    private static final String TAG = "FunctionBeginEvent";
    ...
    public boolean recordTime(){
        if(TextUtils.isEmpty(functionName)){
            LogUtils.bfcExceptionLog(TAG, ErrorCode.CONTROL_NULL_POINTER_PAGE_FUNCTIONNAME);
            return false;
        }
        if(SharedPrefUtils.getKeyLongValue(functionName, -1) == -1){
            SharedPrefUtils.saveKeyLongValue(functionName, SystemClock.elapsedRealtime());
            return true;
        }else{
            LogUtils.bfcExceptionLog(TAG, ErrorCode.CONTROL_PAGE_FUNCTIONNAME_MULTIDEX);
            return false;
        }
    }

    public FunctionBeginEvent setActivity(String activity) {
        this.activity = activity;
        return this;
    }

    public FunctionBeginEvent setFunctionName(String functionName) {
        this.functionName = functionName;
        return this;
    }

    public FunctionBeginEvent setModuleDetail(String moduleDetail) {
        this.moduleDetail = moduleDetail;
        return this;
    }

    public FunctionBeginEvent setExtend(String extend) {
        this.extend = extend;
        return this;
    }
}
